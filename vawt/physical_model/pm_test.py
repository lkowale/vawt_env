...
op_interp_dir = '/home/aa/vawt_env/vawt/physical_model/pitch_optimizer/exps/naca0018_m_8/'
...
class TestVawtPm(unittest.TestCase):
    ...
    def test_get_tforce(self):
        ...
        # blade, wind_direction, wind_speed, tsr, theta, tf
        data = [
            (blade_1, 0, 5, 2, -2.62, 18.07),
            (blade_1, 0, 5, 2, 2.04, 0),
        ...
        for blade, wind_direction, wind_speed, tsr, theta, tf in data:
            wind_vector = bc.get_wind_vector(wind_direction, wind_speed)
            # TSR = ωR/V∞ -> w=TSR*wind_speed/R
            # tsr = rotor_speed * blade.sa_radius / wind_speed
            # set pitch #wind_speed, tsr, rotor_theta, wind_direction
            blade.pitch = blade.get_optimal_pitch(wind_speed, tsr, theta, wind_direction)
            rotor_speed = tsr * wind_speed / blade.sa_radius

            calculated = blade.get_tforce(wind_vector, rotor_speed, theta)
            self.assertAlmostEqual(calculated, tf, 1, 'Wrong Ft calculation')
